...
- 항목 정의:
	...
	- $Res$: 각 기물에 대해 $\text{move\_stack} - 0.5\times\text{stun\_stack}$ 의 합(백은 양수, 흑은 음수).
...
- 착수(PGN) 관점 스코어 예시:
	$$\mathrm{placement\_score}(pgn,player) = \begin{cases} +\mathrm{decay}(v,f,r) &\text{if player==WHITE} \\ -\mathrm{decay}(v,f,r) &\text{if player==BLACK} \end{cases}$$
